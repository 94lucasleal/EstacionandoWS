...
public class TransacaoBD extends CrudBD<Transacao> {
    ...
    public Transacao consultarQrCode(Transacao bean) {
        ...
        try {
            ...
            PreparedStatement pstm = conn.prepareStatement("SELECT * FROM transacao WHERE qrcode = ?");
            ...
            if (rs.next()) {
                ...
                transacaoRetorno.setDta_entrada_real(rs.getString("dta_entrada_real"));
                transacaoRetorno.setDta_saida_prevista(rs.getString("dta_saida_prevista"));
   ...
   public void estonarPagamento(Transacao bean) {
	...
	TimeZone.setDefault(tz);
        ...
        try {
            ...
            PreparedStatement pstm = conn.prepareStatement("SELECT * FROM transacao WHERE dta_entrada > ? AND idusuario = ? AND AND (estornado is null or estornado  <> ?) AND (utilizou_reserva is null or utilizou_reserva  <> ?)");
    ...
    public ArrayList<Transacao> pesquisar(String pesquisa) {
        ...
        try {
            ...
            PreparedStatement pstm = conn.prepareStatement("SELECT * FROM transacao WHERE idestabelecimento = ? AND dta_entrada is not null AND dta_saida is not null");
